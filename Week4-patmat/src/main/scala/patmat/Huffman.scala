...
object Huffman {
  ...
  def until(xs: List[CodeTree]=>Boolean, ys: List[CodeTree]=>List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (xs(trees)) trees
    else until(xs, ys)( ys(trees) )
  }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encodeF(tree: CodeTree)(c: Char): List[Bit] = (tree,c) match {
        case (_: Leaf, _) => Nil
        case (f: Fork, x) if chars (f.left).contains (x) => 0 :: encodeF (f.right) (x)
        case (f: Fork, x) => 1 :: encodeF (f.left) (x)
        case _ => throw new Error ("encode: invalid input")
      }
      text flatMap encodeF(tree)
    }
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
      case (`char`, l) :: xs => return l
      case (_ , _) :: xs     => codeBits(xs)(char)
      case  _                => throw new Error("char not found in table")
    }
   ...
   */
    def convert(tree: CodeTree): CodeTable = tree match {
      case (l: Leaf) => List( (l.char, Nil) ) : CodeTable
